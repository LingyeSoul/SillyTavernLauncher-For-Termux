...
class SillyTavernCliLauncher:
    ...
    def __init__(self):
        ...
        self.update_checker = UpdateChecker(
            current_version=self.launcher_version, mirror=mirror
        )
...
"""
    ...
    def _connect_and_sync(self, server_url):
        ...
        try:
            ...
            if sync_choice.lower() != "n":
                ...
                method_map = {"1": "auto", "2": "zip", "3": "incremental"}
        ...
        while True:
            ...
            try:
                ...
                if choice == "1":
                    ...
                    try:
                        server_choice = input(
                            f"请选择服务器编号 [1-{len(saved_servers)}]: "
                        ).strip()
                        if server_choice.isdigit() and 1 <= int(server_choice) <= len(
                            saved_servers
                        ):
                    ...
                    try:
                        server_choice = input(
                            f"请选择要删除的服务器编号 [1-{len(saved_servers)}]: "
                        ).strip()
                        if server_choice.isdigit() and 1 <= int(server_choice) <= len(
                            saved_servers
                        ):
        ...
        while True:
            ...
            try:
                ...
                if choice == "1":
                    ...
                    print("当前同步配置:")
                    print(
                        f"  实际运行状态: {'运行中' if status['running'] else '已停止'}"
                    )
                    print(
                        f"  配置启用状态: {'启用' if status['config_enabled'] else '禁用'}"
                    )
                    print(
                        f"  状态一致性: {'一致' if status['consistent'] else '不一致'}"
                    )
                    ...
                    # 设置同步服务器端口
                    try:
                        sync_port = status["port"]
                        new_port = input(
                            f"请输入新的端口号 (当前: {sync_port}): "
                        ).strip()
                        if new_port:
    ...
    def update_launcher(self, restart_after=False, allow_dev=False):
        ...
        if allow_dev:
            ...
            result = self.update_checker.check_update_sync(
                timeout=10, allow_dev=allow_dev
            )
            ...
            print(
                f"发现新版本: v{result['current_version']} -> v{result['latest_version']}"
            )
    ...
    def migrate_from_path(self, source_path: str, mode: str = "move"):
        ...
        try:
            ...
            confirm = input(
                f"\n确认从 {source_path} 迁移到 {self.migrator.target_path}？(y/N): "
            ).strip()
            if confirm.lower() != "y":
                print("取消迁移")
                return
...
def main():
    ...
    parser.add_argument(
        "--mode",
        choices=["move", "copy_data", "copy_config", "copy_all"],
        help="迁移模式",
    )
    ...
    if args.command == "install":
        ...
        if args.subcommand == "info":
            ...
            print(
                f"白名单存在: {'是' if os.path.exists(launcher.stCfg.whitelist_path) else '否'}"
            )
        ...
        if args.subcommand == "scan":
            ...
            if args.source_path:
                ...
                print(
                    "  --mode <mode>          - 迁移模式: move, copy_data, copy_config, copy_all"
                )
                print("")
                print("迁移模式:")
